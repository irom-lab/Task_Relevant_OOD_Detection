...
file_version = 'datasets_'
prior_path = 'prior_' + str(file_version)
post_path = 'post_' + str(file_version)


class DataLoader():
    def __init__(self, data_dict, device, batch=16, dataset = "data500"):
        self.device = device
        self.batch = batch
        self.data = {}
        self.position = {}
        for key in data_dict:
            ...
            if option is None:
                option = 'prim_cost'
            self.data[key] = load_data(app=file_name, option=option, dataset=dataset)
            self.position[key] = 0
...
dl = {} # dictionary with key = dataset, value = data
for dataset in datasets:
    dl[dataset] = DataLoader(data_dict, device, batch, dataset = dataset)
...
for i, dataset in enumerate(datasets): 
    ...
    if step == 0:
        ...
        load_weights(model, prior_path+dataset)
        model.init_logvar(-6, -6)
        prior = copy.deepcopy(model)
        save_weights(prior, prior_path+dataset)
        # finetune_params = []
        ...
        load_weights(model, post_path+dataset)
        load_weights(prior, prior_path+dataset)
        optimizer = None

N = 10000
m = torch.tensor(len(dl[datasets[0]].data['post'][0]), dtype=torch.float)
...
for i, dataset in enumerate(datasets): 
    ...
    if step < 2:
        for n in range(1, int(N)+1):
            ...
            if compute_prior:
                x, y = dl[dataset].get_batch('prior')
            else:
                x, y = dl[dataset].get_batch('post')
            ...
            if n % 1000 == 0:
                print()
                x, y = dl[dataset].get_batch('test', batch=test_batch)
                model_output = model(x)
                ...
                if step == 0:
                    save_weights(model, save_file_name=prior_path+dataset)
                else:
                    save_weights(model, save_file_name=post_path+dataset)
        print()

    if step == 2:
        output = []
        x, y = dl[dataset].data['post_prim_costs']
        x = x.to(device)
        # y = y.to(device)
        model.init_xi()
        model_output = model(x)
        ...
        print("Derandomized UB: ", emp_cost + reg_pac_bayes_r)
        print("Derandomized LB: ", emp_cost - reg_pac_bayes_r)

        # print("Maurer", emp_cost + reg_pac_bayes_k)
        # pac_bound = kl_inv_l(emp_cost, Rg_k)
        # print("KL-inv", pac_bound)
        # output: cost, upper bound, lower bound
        output.append((emp_cost, emp_cost + reg_pac_bayes_r, emp_cost - reg_pac_bayes_r))

        np.save('weights/'+post_path+dataset+'.npy', output)